...
get_pte(pde_t *pgdir, uintptr_t la, bool create) {
    ...
    if ((*pdep & PTE_P) == 0) {              // (2) check if entry is not present
        ...
        if (create) {               // (3) check if creating is needed, then alloc page for page table
            ...
            uintptr_t pa = page2pa(new_page); // (5) get linear address of page
            uint32_t cr0 = rcr0();
            if (cr0 & CR0_PG) {
            ...
            *pdep = (uint32_t)pa | PTE_P | PTE_U | PTE_W;  // (7) set page directory entry's permission
        }
    }
